...
def test_regression_gd(sample_data, slope_and_intercept):
    ...
    losses = model.fit(x, y, learning_rate=1e-4, epochs=70000)

    for i in range(len(losses)):
        if i % 10000 == 0:
            logger.info(f'EPOCH {i}, loss={losses[i]}')
    
    logger.info(f'model.weights={model.weights[0][0]:.4f}, model.intercept={model.intercept}')